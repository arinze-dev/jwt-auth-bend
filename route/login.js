...
// 
	...
	if (!correPassword) return res.status(400).json({status:400,message:"wrong password try angin"});
	// creating the token
	const token = jwt.sign({ _id: user._id }, process.env.TOKEN_SECRET);
	// res.header("usertoken", token).send(token);
	res.status(200).json({status:200,id: user._id, name:user.name, email:user.email,token })
